...
has 'start_index' => ( is => 'ro', isa => 'Maybe[Int]', lazy_build => 1 );
sub _build_start_index {
    my $self = shift;
    $self->rest->query->param('startIndex');
}
...
has 'order' => ( is => 'ro', isa => 'Str', lazy_build => 1 );
sub _build_order {
    my $self = shift;
...
sub get_resource {
    ...
    if (defined $self->start_index and $content_type eq 'application/json') {
        return {
