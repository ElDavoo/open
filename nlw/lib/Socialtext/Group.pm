...
sub sorted_user_roles {
    ...
    my @where = ('ugr.user_set_id' => $self->user_set_id);
            ...
            $from .= q{
                LEFT JOIN (
                    SELECT from_set_id AS user_id,
                     ...
                     WHERE into_set_id }.PG_WKSP_FILTER.q{
                       AND from_set_id }.PG_USER_FILTER.q{
                     GROUP BY user_id
                ) wses USING ( user_id )
            };
            $sort = 'count';
    ...
    if ($opts{raw}) {
        ...
        $apply = sub {
            my $row = shift;
            $row->{user} = Socialtext::User->new(user_id => $row->{user_id});
            $row->{role} = Socialtext::Role->new(role_id => $row->{role_id});
            return $row;
        };
    }
    return Socialtext::MultiCursor->new(
