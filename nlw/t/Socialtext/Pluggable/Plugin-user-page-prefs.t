...
# Get/Set
{
    ...
    my $expected = $plugin->get_user_page_prefs(
        workspace_name => $ws->name,
        page_id        => 'something'
    );
...
# Check perms
{
    ...
    my $returned;
    lives_ok {
        $returned = $plugin->set_user_page_prefs(
            workspace_name => $ws->name,
            page_id        => 'something',
            prefs          => { number => 32 }
        );
...
# Settings are page scoped
{
    ...
    $expected = $plugin->get_user_page_prefs(
        workspace_name => $ws->name,
        page_id        => 'page2',
    );
...

{
    ...
    my $expected = $other_plugin->get_user_page_prefs(
        workspace_name => $ws->name,
        page_id        => 'something',
    );
